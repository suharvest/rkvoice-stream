...
- Continue prefill surgery around attention, but do not use the full `attn_residual` slice on current RK3576 runtime. The all-layer `ln1_cattn` integration is accurate and faster than both `ln2_mlp` baseline and the narrower `cattn` route, reaching `979.765 ms` prefill (`1.159x` over baseline). The next concrete step is reducing the remaining attention suffix cost or runner handoff overhead; service promotion still requires streaming TTFA and ASR roundtrip gates, not just hidden/KV parity.
