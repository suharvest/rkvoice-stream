...
- Continue prefill surgery around attention, but do not use the full `attn_residual` slice on current RK3576 runtime. The all-layer `cattn` integration is accurate and faster than the previous `ln2_mlp` hybrid prefill, but only by `1.071x` because `ln1`, attention suffix, and ORT/RKNN handoff costs dominate. The next concrete step is a coarser stable NPU island or a lower-overhead runner; service promotion still requires streaming TTFA and ASR roundtrip gates, not just hidden/KV parity.
