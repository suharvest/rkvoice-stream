...
### 12.5 #3 KV-clear 运行期脏 KV 防护 — 真机 live 验通过

gemma4 server 模式**每请求后无条件** `rknn3_session_clear_kvcache(..., RKNN3_KVCACHE_CLEAR_ALL)`(#3,源在 `models/rk1828/gemma4-patches/full-files/rknn_gemma4.cc`,三处:请求前/请求后/server-loop 收尾)防止跨请求脏 KV 累积溢出 → SIGABRT。

**radxa 真机 live 验(2026-06-22)**:确认设备 gemma4 源码 md5 == committed `17cc4ae` full-files(含 #3),`build-linux.sh -t rk3588 -a aarch64 -d gemma4` 重建(model 目录 mv move-aside 过 build,磁盘 96% 不能 cp),binary md5 `9963bdc68e627566d40731e78aa87a4f`(与既有一致 = 确定性复现,生产 binary 本就含 #3)。

stress:`systemctl stop tts-radxa` 腾 EP → 用生产 `AudioLLMWorker`(1 次 load,Task-1 重试兜底)server 模式**连发 12 请求**,其中 **3 个故意触发错误路径**(不存在的 audio_ref / 近空音频 / 超长 prompt ×400)。结果:**全部 12 请求 `rc=None alive=True`,SIGABRT_SEEN=False**,错误请求后的 good 请求仍正常出 token(末请求 32 token,语义连贯)。证明 #3 真的防住了运行期脏 KV 溢出 SIGABRT。完事 `systemctl start tts-radxa` 恢复 → health `{"status":"ok"}`,EP 无 wedge。

### 12.6 worker load 重试缓解(regime ①)

`rknn3_worker.py` 的 `RKNN3Worker.start()` 与 `AudioLLMWorker.start()` 加 **in-process 重试**:每次 spawn 等 ready,失败(进程 Init 阶段死 / ready 超时 → `WorkerCrashError`)则清理 + 短 backoff(2s/4s)重试,默认 3 次(`start_attempts` 参数或 `RK1828_WORKER_START_ATTEMPTS` env 可配)。N 次仍失败抛 `WorkerStartError`("RK1828 EP may be degraded — a clean host reboot is likely required")。这救 regime ①(冷启握手 race);regime ②(持久 ACK_FAIL 退化)in-process 重试无用、须干净 reboot 重刷固件(**backend 绝不自己 reboot,只给可诉求错误**)。`ProtocolMismatchError` 是确定性版本不符,不重试直接抛。off-device 用 mock worker 的 `MOCK_RK1828_FAIL_COUNT_FILE` 计数器测"前 K 次失败、第 K+1 次成功"+ "一直失败 → WorkerStartError"。
