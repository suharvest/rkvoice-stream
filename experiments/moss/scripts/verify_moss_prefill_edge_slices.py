#!/usr/bin/env python3
"""Verify MOSS hybrid prefill edge ONNX slices against full prefill."""
...
EDGE_OUTPUTS = ["/Add_15_output_0", "/Mul_88_output_0", "global_hidden"]
...
def main() -> int:
    ...
    parser.add_argument("--embedding-onnx", required=True, type=Path)
    parser.add_argument("--final-norm-onnx", required=True, type=Path)
    ...
    parser.add_argument("--work-dir", type=Path, default=Path("/tmp/moss-edge-slices"))
    parser.add_argument("--json-out", type=Path)
    args = parser.parse_args()

    augmented = args.work_dir / f"moss_tts_prefill.edge_outputs.s{args.seq_len}.onnx"
    ...
    add15, mul88, global_hidden = full.run(EDGE_OUTPUTS, {"input_ids": input_ids, "attention_mask": attention_mask})
    full_ms = (time.perf_counter() - t0) * 1000.0

    embedding = _session(args.embedding_onnx, args.threads)
    t0 = time.perf_counter()
    embedding_out = embedding.run(None, {"input_ids": input_ids})[0]
    embedding_ms = (time.perf_counter() - t0) * 1000.0

    final_norm = _session(args.final_norm_onnx, args.threads)
    t0 = time.perf_counter()
    ln_f = final_norm.run(None, {"/Mul_88_output_0": mul88})[0]
    final_out = np.asarray(ln_f, dtype=np.float32) * attention_mask[:, :, None].astype(np.float32)
    final_ms = (time.perf_counter() - t0) * 1000.0

    outputs = {
        "embedding_prefix": _metrics(add15, embedding_out),
        "final_norm": _metrics(global_hidden, final_out),
    }
    passed = all(item["finite"] and item["rel_l2"] <= 1e-5 and item["cosine"] >= 0.99999 for item in outputs.values())
    report = {
        "model_dir": str(args.model_dir),
        "embedding_onnx": str(args.embedding_onnx),
        "final_norm_onnx": str(args.final_norm_onnx),
        "text": args.text,
        "voice": args.voice,
        "seq_len": args.seq_len,
        "actual_len": actual_len,
        "latency_ms": {
            "full_prefill_augmented": round(full_ms, 3),
            "embedding_prefix": round(embedding_ms, 3),
            "final_norm": round(final_ms, 3),
        },
        "outputs": outputs,
