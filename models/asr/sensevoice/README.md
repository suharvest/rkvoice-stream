...
| CMVN | `(lfr + add) * scale` (560-dim, from `am.mvn`) — **applied** (device-validated default). NOTE: on FP32 onnxruntime removing CMVN *looks* better (mean CER 0.048→0.032; the lovemefan ONNX self-normalizes), but on the real RK3576 fp16 NPU it is NOT a clean win — CMVN/noCMVN each collapse on a different subset of short zh clips (per-sample fp16 instability). Revisit only with a proper on-device corpus CER. |
